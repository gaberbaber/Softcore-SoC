...
/**
 * Inclination/Orientation sensing using ADXL362 accelerometer
 * Determines board orientation: 0°, 90°, 180°, or 270°
 * Uses LEDs 15:12 to indicate orientation
 */

void board_orientation_check(SpiCore *spi_p, GpoCore *led_p) {
    const uint8_t RD_CMD = 0x0b;
    const uint8_t DATA_REG = 0x08;
    const float raw_max = 127.0 / 2.0; // ADXL362 +/-2g, 8-bit
    ...
    while (1) {
        // Activate SPI and read x/y/z 8-bit values
        spi_p->assert_ss(0);
        ...
        // Convert raw values to normalized g's (-1 to +1)
        x = (float)xraw / raw_max;
        y = (float)yraw / raw_max;
        ...
        if (fabs(x) > fabs(y)) {
            // x-axis dominates
            if (x > 0) led_pattern = 0x01;  // 0°
            else        led_pattern = 0x04;  // 180°
        } else {
            // y-axis dominates
            if (y > 0) led_pattern = 0x02;  // 90°
            else       led_pattern = 0x08;  // 270°
        }
 
        led_p->write(led_pattern);
...
int main() {
   ...
   while (1) {
       board_orientation_check(&spi, &led);
      //show_test_id(1, &led);
      //led_check(&led, 16);
      //sw_check(&led, &sw);
      //show_test_id(3, &led);
